...
const ProgramsCard: React.FC<Props> = ( {id, training_name, category, duration, provider, reason, total_cost, onDelete} ) => {
        ...
        <div className="px-5 py-7 bg-white shadow-xs shadow-secondary/30 flex flex-col">
            <div className="w-full flex items-center mb-5 justify-between">
               ...
               <div className="px-2 py-1">
                 <label className="text-xs font-bold text-secondary/50">N$ {total_cost.toLocaleString("fr-FR", {minimumFractionDigits: 2, maximumFractionDigits: 2})}</label>
               </div>
            </div>
            <label className="text-xs whitespace-nowrap mb-2 text-secondary/40 overflow-hidden text-ellipsis">{category}</label>
            ...
            <div className="mt-4 flex items-center justify-between">
               <label className="text-xs text-secondary/60">{reason}</label>
